...
from .scanner.tls import RawResult, grab
...
def run_scan(triggered_by="cli", existing_scan_id=None):
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {pool.submit(one, s):s for s in services}
        for future in as_completed(futures):
            service=futures[future]
            try: service,raw=future.result()
            except Exception as exc: raw=RawResult(False,f"Ошибка сканирования: {exc}")
            try: result=analyze(raw,service,thresholds)
            except Exception as exc:
                raw=RawResult(False,f"Ошибка анализа: {exc}"); result=analyze(raw,service,thresholds)
